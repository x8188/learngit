...
with open("00_NCNR_CG_Contrib.csv","w") as csvfile: 
    ...
    writer.writerow(a)
    #写入多行用writerows
    for i,data in enumerate(task_to_contrib_scores):
      writer.writerow([LMR_Index[i]])
      writer.writerows(data.T)

simple_upload("00_NCNR_CG_Contrib.csv",'./')
...
with open("00_NCNR_CG_Contrib.csv","w") as csvfile: 
    ...
    writer.writerow(a)
    #写入多行用writerows
    for i,data in enumerate(task_to_hyp_contrib_scores):
      writer.writerow([LMR_Index[i]])
      writer.writerows(data.T)

simple_upload("00_NCNR_CG_HyperContrib.csv",'./')
